...
| Avoid long-lived Azure credentials in GitHub | GitHub issues an OIDC token whose issuer, audience, immutable repository identity, environment, run and commit are validated before Azure token exchange. | GitHub OIDC receipt and Azure federated-credential read-back | The exact default GitHub subject must match. The workflow receives `id-token: write`; no GitHub secret stores an Azure client secret or certificate. |
...
| Produce bounded, reviewable evidence | Runtime output is reduced to allowlisted JSON in one size-bounded GitHub artifact. Public job-log receipt fallback is disabled. The identity receipt contains a SHA-256 commitment instead of raw user or tenant identifiers. | Receipt schema, file size, SHA-256 and cross-record checks | Missing, oversized, stale, ambiguous, cross-run or identity-mismatched evidence fails closed. No private key, passphrase, token, raw identity or raw sign-in payload is published. |
| Remove transient access and compute | The runner is registered `--ephemeral`; runtime credentials are shredded; the ACI container is deleted; runner deregistration is queried; the lab CA policy returns to report-only. | Live GitHub/Azure queries and policy journals | Zero matching ACI containers and zero matching repository runners; Conditional Access restoration must be exact. |
