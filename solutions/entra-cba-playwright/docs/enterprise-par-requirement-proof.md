...
| Remove transient access and compute | The runner is registered `--ephemeral`; cloud credentials remain in process memory and the workflow rejects credential files; the transient artifact is deleted; the ACI container is deleted; runner deregistration is queried; the lab CA policy returns to report-only. | Live GitHub/Azure queries, schema-v3 receipt and policy journals | `credentialFilesWritten=false`; zero matching artifacts, ACI containers and repository runners; Conditional Access restoration must be exact. |
